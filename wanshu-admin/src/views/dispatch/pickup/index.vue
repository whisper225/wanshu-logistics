<template>
  <PageContainer title="取件作业管理">
    ...
    <el-table :data="tableData" v-loading="loading">
      ...
      <el-table-column prop="pickupTime" label="取件时间" width="180" />
      <el-table-column prop="status" label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="pickupStatusType(row.status)" size="small">
            {{ pickupStatusMap[row.status] }}
          </el-tag>
        </template>
      </el-table-column>
...
const pickupStatusMap: Record<number, string> = { 0: '待分配', 1: '已分配', 2: '已取件', 3: '已取消' }
const pickupStatusType = (s: number) => s === 2 ? 'success' : s === 3 ? 'danger' : s === 1 ? 'warning' : 'info'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await dispatchApi.getPickupTaskList({ ...searchForm, ...pagination })
...
const handleAssign = async (row: any) => {
  ...
  await dispatchApi.assignPickupTask(row.id, Number(value))
  ElMessage.success('分配成功')
  loadData()
}

const handleCancel = async (row: any) => {
  const { value } = await ElMessageBox.prompt('请输入取消原因', '取消取件')
  await dispatchApi.cancelPickupTask(row.id, value)
