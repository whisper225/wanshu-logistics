	...
	<view class="task-list">
		...
		>
			...
			<view class="card-body">
				<view class="info-row" v-if="item.waybillNumber">
					<text class="label">运单号</text>
					<text class="value">{{ item.waybillNumber }}</text>
				</view>
				<view class="info-row" v-if="item.receiverName">
					<text class="label">收件人</text>
					<text class="value">{{ item.receiverName }} {{ item.receiverPhone }}</text>
				</view>
				<view class="info-row" v-if="item.receiverAddress">
...
<script setup>
	import { ref } from 'vue'
	import { getDeliveryTasks } from '../../api/courier.js'
	...
	const loadTasks = async () => {
		loading.value = true
		try {
			const data = await getDeliveryTasks(props.status)
			list.value = data || []
		} catch (e) {
